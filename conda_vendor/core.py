...
class CondaChannel:
    def __init__(self, platforms=["linux-64", "noarch"], channel_path=pathlib.Path("./")):
        ...
        for platform in platforms:
            self.channel_info[platform] = {'dir': self.local_channel / platform, "url":  self.base_url + platform + "/repodata.json"  }
    ...
    def conda_vendor_artifacts_from_solutions(self, conda_solution, requests=requests):
        ...
        for platform, info in self.channel_info.items():
            ...
            pkg_names = [
                link["dist_name"] for link in conda_solution if link["platform"] == platform
            ]
            # print(f'pkg_names: {pkg_names}')

            pkg_correct_extensions = self.create_dot_conda_and_tar_pkg_list(pkg_names, platform)  
    ...
    def download_binaries(self, manifest_dict : dict, requests=requests) -> None:
        ...
        for resource in resources_list:        
            ...
            self.download_and_validate(output_path / resource["name"], 
...
#TODO: this should also make or use a function that makes a local_yaml. 
def conda_vendor(environment_yml, outpath=pathlib.Path("./"), conda_lock_wrapper=CondaLockWrapper(), requests=requests):
    ...
    manifest = conda_channel.conda_vendor_artifacts_from_solutions(conda_solution=link_actions, requests=requests)
