...
def test_CondaChannel_fetch_and_filter(mock_download, conda_channel_fixture):
    ...
    mock_download.return_value = mock_response(
            json_data=fake_live_repo_data_json)

    result = conda_channel_fixture.fetch_and_filter('fake_subdir',
            fake_extended_repo_data)
...
def test_CondaChannel_get_all_repo_data(
        mock_data,
        mock_fetch,
...
def test_CondaChannel_local_dir(mock, conda_channel_fixture):
    ...
    expected = (conda_channel_fixture.channel_root /
            mock_local_dir_name / test_subdir)
    result = conda_channel_fixture.local_dir(
            chan=test_channel_name,
            subdir=test_subdir)
...
def test_CondaChannel_make_local_dir(mock, conda_channel_fixture):
    ...
    expected_path = (conda_channel_fixture.channel_root /
            mock_local_dir_name / test_local_dir_subdir)

    test_channel_name = "TEST_CHANNEL_NAME"
    mock.return_value = expected_path
    result = conda_channel_fixture.make_local_dir(
            chan=test_channel_name,
            subdir=test_local_dir_subdir)
...
def test_CondaChannel_write_repo_data(
        mock_get,
        mock_write,
        conda_channel_fixture):
    platform = conda_channel_fixture.platform
...
def test_CondaChannel_download_arch_binaries(
    ...
    mock_download_and_validate.assert_called_with(
            *expected_download_and_validate_calls)
...
def test_create_manifest(mock_create_manifest, conda_channel_fixture):
    ...
    result = create_manifest(conda_channel_fixture,
            manifest_filename=test_manifest_name)
    mock_create_manifest.assert_called_once_with(
        manifest_filename='FAKE_NAME')
...
def test_get_local_environment_yaml(conda_channel_fixture):
    ...
    result = get_local_environment_yaml(conda_channel_fixture,
